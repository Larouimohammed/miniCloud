...
consulinstall:
	...
	@ sudo docker run -d --name=consul-server -p 8500:8500 -p 8600:8600/udp -p 8600:8600/tcp -p 8500:8500/udp -p 8500:8500/tcp -v /home/ubuntu/miniCloud:/consul/config consul:1.15.4 agent -server -bootstrap-expect=1 -ui -node=server-1 -config-file=/consul/config/consul-config.json -client=0.0.0.0
